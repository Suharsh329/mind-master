...
function loadGame() {
    ...
    if (savedGame) {
        ...
        if (!gameState.hasOwnProperty('activePlayer') && gameState.players.length > 0) {
         ...
         if (!gameState.hasOwnProperty('currentPlayer')) { // For older saves
...
export function renderBoard() {
    ...
    if (!gameState.boards || gameState.boards.length === 0 || !gameState.boards[gameState.currentBoard]) {
        boardContainer.innerHTML = '<p style="text-align:center;">No game data loaded or board is empty. Try starting a new game or editing game data.</p>';
...
function openAdjustScoreModal(playerIndex) { // playerIndex can be the active player or undefined initially
