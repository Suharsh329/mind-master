...
let editorModal,
  ...
  newQuestionPointsInput,
  newQuestionTextInput,
  newQuestionAnswerInput;
...
function addNewBoard() {
  ...
  const emptyCategoriesArray = Array(5)
    .fill()
    .map((_, i) => `Category ${i + 1}`);
  const emptyQuestionsArray = Array(5)
    .fill(null)
    .map(() =>
      Array(5)
        .fill(null)
...
function populateBoardSelectDropdowns() {
  ...
  boardSelectCategory.value =
    selectedCategoryBoard && tempBoardsData[selectedCategoryBoard] ? selectedCategoryBoard : tempBoardsData.length > 0 ? "0" : "";
  boardSelectQuestion.value =
    selectedQuestionBoard && tempBoardsData[selectedQuestionBoard] ? selectedQuestionBoard : tempBoardsData.length > 0 ? "0" : "";
...
function deleteCategory(boardIndex, categoryIndex) {
  ...
  if (
    confirm(
      `Are you sure you want to delete category: ${tempBoardsData[boardIndex].categories[categoryIndex]}? This will also remove all its questions.`,
    )
  ) {
...
function addNewCategory() {
  ...
  if (tempBoardsData[boardIndex].categories.length >= 5) {
    // Assuming a fixed max of 5 for now based on game structure
...
function updateQuestionsList() {
  ...
  if (
    isNaN(boardIndex) ||
    boardIndex < 0 ||
    boardIndex >= tempBoardsData.length ||
    isNaN(categoryIndex) ||
    categoryIndex < 0 ||
    categoryIndex >= tempBoardsData[boardIndex].questions.length
  ) {
...
function editQuestionPart(boardIndex, categoryIndex, questionIndex, partKey, element, isTextarea = false) {
  ...
  const saveEdit = () => {
    ...
    if (partKey === "points" && (isNaN(newValue) || newValue < 0)) {
      // Allow 0 points
      alert("Points must be a non-negative number.");
      updateQuestionsList();
      return;
    }
    if (newValue !== originalValue || (partKey === "points" && newValue !== originalValue)) {
      // Fix for points not saving if changed to 0
      if (partKey === "points" && isNaN(newValue)) {
        // If points cleared, treat as 0 or handle error
...
function addNewQuestion() {
  const boardIndex = parseInt(boardSelectQuestion.value);
  const categoryIndex = parseInt(document.getElementById("category-select-questions").value);
  if (isNaN(boardIndex) || isNaN(categoryIndex) || !tempBoardsData[boardIndex] || !tempBoardsData[boardIndex].questions[categoryIndex]) return;
    ...
    for (const board of tempBoardsData) {
      ...
      for (let i = 0; i < board.questions.length; i++) {
        if (board.questions[i].length !== 5) {
          alert(
            `Error: Category "${board.categories[i]}" on board "${board.categories[0]}..." does not have 5 questions. Please adjust before saving.`,
          );
          return;
        }
      }
  ...
  [newQuestionPointsInput, newQuestionTextInput, newQuestionAnswerInput].forEach((input) => {
    input.addEventListener("keypress", (e) => {
      if (e.key === "Enter" && (input.tagName !== "TEXTAREA" || e.shiftKey === false)) {
        if (
          input === newQuestionAnswerInput ||
          (input === newQuestionTextInput && newQuestionAnswerInput.value) ||
          (input === newQuestionPointsInput && newQuestionTextInput.value && newQuestionAnswerInput.value)
        ) {
          e.preventDefault(); // Prevent form submission if inside a form
          addNewQuestion();
        } else if (input.tagName !== "TEXTAREA") {
          // For points and text input, advance focus on enter
